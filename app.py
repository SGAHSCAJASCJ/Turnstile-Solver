...
def create_app(headless: bool, thread: int, page_count: int) -> FastAPI:
    server = TurnstileAPIServer(headless=headless,thread=thread, page_count=page_count)
    return server.app

if __name__ == '__main__':
    headless = True
    # 浏览器实例数量(最大不要超过cpu核心数)
    thread = 1
    # 每个浏览器实例的页面数量(建议值为1)
    page_count = 1
    # 绑定主机a
    ...
    # 绑定端口
    port = 8000
    app = create_app(headless=headless,thread=thread,page_count=page_count)
